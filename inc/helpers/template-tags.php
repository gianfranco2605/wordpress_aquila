...
function aquila_posted_by() {
    $byline = sprintf( 
        esc_html_x( ' .  by %s', 'post_author', 'aquila' ) ,
...
function aquila_the_excerpt( $trim_character_count = 0 ) {
    ...
    $excerpt = wp_strip_all_tagas( get_the_excerpt() );
...
function aquila_excerpt_more() {
    ...
    if ( ! is_single() ) {
        ...
        $more = sprintf( '<a class="aquila-read-more text-white" href="%1$s" ><button class="mt-4 btn btn-info">%2$s</button></a>', get_permalink( get_the_ID() ), __( 'Read More', 'aquila' ) );
    }
